...
- **Mislukte login produceert een WARNING-logregel met IP-adres voor fail2ban** (sluit #130) — `lib/insigne/users.py:log_failed_login_attempt` emit een log van de vorm `failed login attempt for email=<x> from <ip>` op WARNING-niveau. Zowel de HTML-loginflow (`/login` POST) als de JSON-API (`/api/auth/token`) roepen het aan op `authenticate(...) → None`. Een geslaagde login schrijft niets naar de WARNING-stream zodat fail2ban-filters geen false positives krijgen. Voorbeeld-filter regex: `failregex = failed login attempt for email=\S+ from <HOST>`. 4 unit-tests op de helper + 2 integratietests op de HTML-flow.
