...
- **Origin-header CSRF-check als tweede laag bovenop SameSite=Lax** (sluit #99) — een nieuwe Starlette-middleware (`api/main.py:origin_csrf_check`) blokkeert state-changing HTML-requests (POST/PUT/DELETE/PATCH op routes buiten `/api/`) waarvan de `Origin`-header niet matched met `config.base_url`. Browsers sturen `Origin` altijd op cross-site form submits — die worden nu hard geweigerd met `403`, ook in browsers die `SameSite=Lax` niet of slecht handhaven. Niet-browser-clients (curl, server-to-server, TestClient) sturen geen `Origin` en blijven werken; de JSON-API onder `/api/` blijft buiten beschouwing omdat die bearer-token-auth gebruikt en niet cookie-driven is. Audit van GET-routes bevestigde dat alleen twee e-maillink-bevestigingsflows state muteren via GET (`/register/confirm/{code}` en `/profile/email-change/confirm/{token}`) — beide vereisen een eenmalig geheim token uit de e-mail, dus CSRF-resistent by design. Nieuwe sectie in `CLAUDE.md`: "State-changing HTML endpoints rely on two CSRF defences".
