...
- **Poster-ontwerper — fundament** (#132, fase 1, sluit #152) — een nieuwe `/posters`-sectie waarmee je printbare insigneposters maakt. Deze eerste fase levert het mechanisme: een ontwerpscherm met een parameterpaneel (titel, subtitel, kop-/voettekst, groep, speltak, lettergroottes, papierformaat A4→A0 en oriëntatie) en een live voorbeeld dat exact toont hoe de poster over meerdere pagina's afdrukt. Het voorbeeld gebruikt **paged.js** (lokaal gevendord, MIT) zodat de paginaverdeling in het voorbeeld gelijk is aan de afdruk; **Printen** opent de poster in een eigen venster en print via de browser (Opslaan als PDF). Posters worden **opgeslagen** — persoonlijk, of (als speltak-/groepsleider) op speltak- of groepsniveau, waar ze beschikbaar zijn voor de leden; een basistemplate per type (bewerkbare YAML-bestanden in de data-map) is het startpunt. De ontwerper werkt op desktop/tablet en weigert met een melding op te kleine schermen. De echte posterinhoud per type volgt in fase 2–4 (#153–#155).
...
- **Poster-ontwerper — YAML-definitie, sjabloonvelden & export/import** (#132) — een poster wordt opgeslagen als een **YAML-document** (de bron van waarheid). Je kunt een poster **exporteren** (`.yml`) en weer **importeren**. Tekstvelden (titel, kop-/voettekst, …) ondersteunen sjabloonvelden: `{{ user.name }}` (de ingelogde gebruiker) en `{{ date }}` / `{{ time }}` worden bij het renderen ingevuld. Dat gebeurt in een **sandbox** met een vaste, kleine context — veilig, ook voor gedeelde speltak-/groepsposters. Beschikbaar: `{{ user.name }}`, `{{ datum }}` / `{{ date }}`, `{{ tijd }}` / `{{ time }}` en `{{ url }}` (de basis-URL). (`{{ lid }}` voor leden van een speltak volgt met de speltak- en aftekenposter.)

- **Poster-ontwerper — insigne-indeling & één-pagina-modus** (#132) — op de insigneposter vormen de niveaus van een insigne samen één rij (zoals de Scouting-vaardigheidsinsigneposter), en de insignes worden **per kolom** geplaatst (kolom 1 van boven naar beneden, dan kolom 2, enz.). Standaard wordt **alles op één pagina** gepast (de inhoud schaalt mee, het gekozen papierformaat blijft behouden); met **"Meerdere pagina's toestaan"** stroomt het over meerdere pagina's. De knoppen Opslaan en Printen hebben nu pictogrammen.

- **Poster-ontwerper — klik-om-te-bewerken** (#132) — voor de insigneposter is er geen stappenwizard meer: je **klikt een element in de poster aan** (titel, subtitel, koptekst, voettekst, insigne-blok of de pagina-achtergrond) en past dán precies dat element aan. Een vaste balk bevat naam + Opslaan / Printen / Exporteren; de pagina (papierformaat, oriëntatie, achtergrond) bewerk je door op de lege pagina te klikken. Tekstelementen hebben nu ook **lettertype** en **kleur** (naast lettergrootte).

- **Poster-ontwerper — element-gerichte definitie, niveaus, achtergrond** (#132) — de posterdefinitie is element-gericht (`elements: { title, subtitle, header, footer, badge_block, background }`, elk met eigen opmaak). De insigneposter kan nu **meerdere niveaus** tegelijk tonen (`niveaus: [1,2,3]` → één afbeelding per niveau per insigne), een **lege insigne-selectie betekent "alle gewone + buitengewone insignes"**, en er is een **achtergrond** (effen of verloop) met kleuren. De systeem-basistemplates staan als bewerkbare YAML in de data-map.
