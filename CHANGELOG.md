...
- **Cross-group lidmaatschap-injectie via lidmaatschapsverzoek geblokkeerd** — een `MembershipRequest` draagt `group_id` en `speltak_id` als losse velden en er werd nergens gecontroleerd dat de speltak bij de groep hoort. Daardoor kon een aanvrager een verzoek maken met `group_id` = een groep die hij zelf beheert (met `allow_any_user_to_create_groups` maakt elke gebruiker zo'n groep aan) en `speltak_id` = een speltak van een *andere* groep, dat verzoek zelf goedkeuren (`can_manage_group` slaagt op de eigen groep) en zo als goedgekeurde scout in de speltak van die andere groep belanden. `create_membership_request` weigert nu met `ValueError("speltak_group_mismatch")` als `speltak.group_id != group_id`, en `approve_membership_request` doet dezelfde controle nogmaals als vangnet vóór het schrijven van het lidmaatschap. Gevonden in de tweede reviewronde (sibling van de eerder gedichte cross-group-schrijfacties). 4 nieuwe tests.

- **Ingelogde sessie honoreert nu de accountstatus** — `_get_current_user` (het enige auth-pad van de HTML-laag) haalde de gebruiker op zonder diens `status` te controleren. Omdat JWT's stateless zijn en 30 dagen leven, zou het op non-actief zetten van een account (schorsen/soft-delete) pas effect hebben wanneer het token verloopt. De helper weigert nu tokens tenzij de gebruiker `status == "active"` heeft — vandaag een no-op (tokens worden alleen voor actieve gebruikers uitgegeven), maar de vereiste vangrail voordat een schorsingsfunctie landt. Het dode `api/deps.py` (een oude bearer-token-helper die sinds het verwijderen van de JSON-API in v1.2.0 nergens meer werd geïmporteerd, mét een afwijkende status-check) is verwijderd zodat er één auth-bron is. 2 nieuwe tests.

- **PDF-export escapet nu gebruikersinvoer (ReportLab-markup-injectie)** — bij de PDF-export werden de scoutnaam, de aftekenaar-naam (`signed_off_by`) en de vrije-tekst-notities ongeëscaped in een ReportLab `Paragraph` gezet, dat een mini-XML-opmaaktaal parseert. Een `<` of `&` in die velden liet de export crashen (500), en een `<img src="file://…">` in een notitie kon ReportLab een lokaal bestand laten inlezen in de eigen PDF van de gebruiker. Deze velden worden nu met `xml.sax.saxutils.escape()` ontdaan van opmaak vóór ze in een `Paragraph` gaan; catalogus-teksten (insignetitels, eisen, labels) zijn vertrouwd en blijven ongewijzigd. 1 nieuwe test.

- **Service worker leegt de cache bij uitloggen (gedeeld apparaat)** — de service worker leegde de runtime-cache alleen bij een versie-bump, niet bij uitloggen. Op een gedeeld/familie-apparaat kon een volgende gebruiker daardoor offline nog de gecachte (geauthenticeerde) pagina's van de vorige gebruiker openen. De pagina stuurt nu bij uitloggen een `logout`-bericht waarop de worker `caches.delete(RUNTIME_CACHE)` uitvoert. `VERSION` naar `v7` (leegt de oude cache ook bij deploy). Bewust géén stop op het cachen van `no-store`-pagina's: dat zou de offline modus op je eigen telefoon breken (dan zou het openen van de app zonder verbinding op de offline-pagina uitkomen i.p.v. je homepage) — network-first serveert altijd een verse kopie zodra er verbinding is, en het legen-bij-uitloggen dekt het gedeelde-apparaat-scenario. 5 nieuwe tests.

- **HTTP-beveiligingsheaders toegevoegd** — de app zette voorheen geen enkele beveiligingsheader. Er is nu een middleware die op elke response `Content-Security-Policy`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff` en `Referrer-Policy: same-origin` zet. De CSP is *pragmatisch*: omdat de UI inline `<script>`-blokken en Alpine.js gebruikt (dat zijn expressies via `new Function` evalueert), staat `script-src` `'unsafe-inline'` en `'unsafe-eval'` toe — een strikte nonce/hash-CSP zou het herschrijven van alle inline scripts vergen. Wat het wél oplevert: `frame-ancestors 'none'` + `X-Frame-Options: DENY` (clickjacking-bescherming op de cookie-geauthenticeerde formulieren), plus `object-src 'none'`, `base-uri 'self'` en `form-action 'self'`. `worker-src` staat `blob:` toe zodat de ALTCHA-captcha zijn proof-of-work-worker kan starten. HSTS wordt bewust niet hier gezet — dat hoort bij de TLS-terminerende proxy. 6 nieuwe tests.

- **Groepsbeheer gehard: zoek-auth, rol-whitelist en toestemming bij lid toevoegen** — drie kleinere hardening-punten in `html_groups.py`. (1) `GET /groups/search` vereist nu authenticatie (gaf voorheen de volledige groepenlijst — id/naam/slug — aan anonieme bezoekers). (2) De `role`-formuliervelden op de speltak-endpoints worden nu tegen een whitelist (`{"scout","speltakleider"}`) gecontroleerd: een onbekende rol uit een gemanipuleerde POST valt terug op de laagst geprivilegieerde `scout` (bij `set-role` wordt een ongeldige rol genegeerd). (3) "Lid toevoegen" (`members/add`, zowel groep als speltak) maakte voorheen direct een *goedgekeurd* lidmaatschap aan — daarmee kon een leider een willekeurige gebruiker zonder diens toestemming in de groep/speltak trekken (en zo toegang tot diens voortgang krijgen). Het gebruikt nu dezelfde uitnodigingssemantiek als de invite-knoppen: een *pending* lidmaatschap dat de betrokkene eerst moet accepteren, met een uitnodigingsmail; een reeds goedgekeurd lid wordt niet gedegradeerd. 8 nieuwe tests.

- **ALTCHA proof-of-work captcha op `/register` en `/contact`** — de zelfgebouwde rekensom-captcha op het contactformulier was door elke bot triviaal op te lossen (twee getallen optellen) en bood dus nauwelijks bescherming; `/register` had helemaal geen captcha. Beide endpoints gebruiken nu [ALTCHA](https://altcha.org/): een zelf-gehoste, privacyvriendelijke proof-of-work-captcha (geen cookies, geen fingerprinting, geen externe calls). De browser-widget lost een PoW op die door ons is ondertekend met een van `jwt.secret_key` afgeleide sleutel; de server verifieert de oplossing en accepteert elke oplossing maar één keer (replay-bescherming). Instelbaar in `config.yml` onder `captcha` (`enabled`, `complexity`). Let op: ALTCHA vereist JavaScript in de browser. Nieuwe dependency `altcha` + gevendorde widget `frontend/static/vendor/altcha.min.js` (altcha@1.5.1, MIT). 22 nieuwe tests.
...
- **Zelf-goedkeuring van lidmaatschapsverzoeken geblokkeerd** — `POST /requests/{id}/approve` en `/reject` controleerden alleen dat de aanvrager was ingelogd, niet dat die de betreffende groep beheert. Omdat elke gebruiker via `POST /groups/join` een lidmaatschapsverzoek voor een willekeurige groep kan aanmaken (en het eigen `req_id` op de homepage ziet), kon iedereen zichzelf als lid/scout van elke groep of speltak goedkeuren — de UI verborg alleen andermans verzoeken, de mutatie zelf was niet geautoriseerd. Bovendien kon een willekeurige gebruiker andermans verzoeken goedkeuren of weigeren door `req_id`'s af te lopen. `approve_membership_request` en `reject_membership_request` weigeren nu met `ValueError("forbidden")` tenzij de beoordelaar de groep van het verzoek beheert (`can_manage_group`); dit dekt zowel groeps- als speltakverzoeken, overeenkomstig de bestaande groepsleider-scoping in `list_all_pending_requests_for_leader`. 7 unit- + 3 integratietests.
...
- **Rate limiting op de openbare e-mailversturende endpoints** — `POST /register`, `/forgot-password` en `/contact` versturen elk een e-mail zonder login en waren ongelimiteerd, waardoor ze te misbruiken waren om andermans inbox (of die van de beheerders) te bombarderen en de SMTP-relay te belasten. Er is nu een per-IP rate limit (via `slowapi`) op alle drie, instelbaar in `config.yml` onder `rate_limit` (standaard `register`/`forgot_password`: `5/hour`, `contact`: `10/hour`; met een `enabled`-schakelaar). De limiet keyt op `request.client.host` — dat is alleen het echte client-IP wanneer `server.forwarded_allow_ips` is ingesteld (uvicorn herschrijft dan uit `X-Forwarded-For`); dezelfde afhankelijkheid als de fail2ban-loginregel. Bewust géén eigen `X-Forwarded-For`-parsing, want dat zou spoofing mogelijk maken buiten een vertrouwde proxy. Nieuwe dependency `slowapi`. 5 nieuwe tests.

- **Account-enumeratie via login-timing gedicht** — `authenticate` voerde de (trage) bcrypt-vergelijking alleen uit als het account bestond; bij een onbekend of niet-actief e-mailadres keerde de functie meteen terug. Het responstijdverschil verklapte zo of een e-mailadres geregistreerd was, wat de enumeratie-bestendige registratieflow ondermijnde. Het niet-gevonden-pad voert nu een dummy-bcrypt-vergelijking uit (`verify_password_dummy` tegen een vaste hash met dezelfde cost-factor), zodat beide paden even lang duren. 3 nieuwe tests.
