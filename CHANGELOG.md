...
- **PWA-wrapper: installeer Insigne Systeem als app** (sluit #101) — een dunne PWA-laag op de bestaande HTMX/Jinja-UI: Web App Manifest (`/static/manifest.webmanifest`) met Scouting-groene `theme_color`, 192/512/512-maskable icons, een service worker (`/sw.js`, vanaf de root geserveerd zodat hij `scope: /` mag claimen) die de statische shell pre-cacht (CSS, icons, manifest) en HTML-responses network-first behandelt, plus iOS-meta-tags (`apple-touch-icon`, `apple-mobile-web-app-capable`). Op Android (Chrome/Edge) krijgt de gebruiker de "App installeren"-prompt; op iOS Safari het standaard *Voeg toe aan beginscherm*-pad. Een nieuwe `/install`-pagina geeft platform-specifieke instructies (bereikbaar vanuit de footer); `/offline` is de fallback wanneer het netwerk weg is. POST/PUT/DELETE wordt nooit gecached, en `/api/*` (toekomst-proof, ook al is de JSON-API in #117 verwijderd) staat op de network-only allowlist. 19 nieuwe integratietests rond manifest-velden, SW-content, icon-PNG-headers, base.html-meta-tags, install- en offline-pagina.

- **Offline alleen-lezen modus voor op kamp** (#101) — wie offline is (bv. op een kamplocatie) kan nu nog steeds: álle insignes en alle eisen op alle niveaus inzien, de eigen voortgang zoals laatst bekeken, en (als leider) het laatst bekeken speltak-overzicht. Bewerken is dan uitgeschakeld: een vaste banner ("Je bent offline — alleen-lezen modus") verschijnt, aftekenknoppen/formulieren worden grijs en niet-klikbaar, en een `htmx:beforeRequest`-vangnet annuleert elke wijzigende request zolang er geen verbinding is. Schermen die niet offline kunnen werken — Aftekeningen, Groepsbeheer, Admin — vallen terug op een nette "werkt niet offline"-pagina (`/offline/disabled`, mee-pre-gecacht); het leider-voortgangsoverzicht (`…/progress`) blijft wél beschikbaar. Een knop **"Maak offline beschikbaar"** op `/install` warmt de hele catalogus (`/offline/manifest.json`) vooraf in de cache, zodat ook nog niet bezochte insignes offline te bekijken zijn. SW-cacheversie `v2` → `v3`.

### Verbeteringen

- **Niveau-keuze op een insignepagina is nu client-side** (#101) — een reguliere insignepagina (`/badges/{slug}`) rendert voortaan altijd alle drie de niveaus in één response; de niveau-tabs op mobiel wisselen client-side (Alpine + `history.replaceState`) in plaats van naar `?niveau=N` te navigeren, en de mobiele redirect is weg. Idem voor de speltak-tabs van een jaarinsigne (openen nu client-side het juiste niveau-`<details>`). Eén cachebare URL per insigne in plaats van vier, wat het offline vooraf-downloaden simpel maakt — en het wisselen van niveau voelt direct, ook online.

- **HTMX en Alpine.js lokaal gehost en pre-gecacht** — de twee kern-libraries werden eerder vanaf een CDN (`unpkg.com` / `cdn.jsdelivr.net`) geladen. De service worker slaat cross-origin requests over, dus offline laadde de gecachete HTML wel maar bleef de pagina dood: geen step-card-aftekenen, dropdowns, inline-notities of HTMX-swaps. HTMX 2.0.4 en Alpine 3.14.1 staan nu vendored onder `/static/vendor/`, worden mee-pre-gecacht in de SW-shell (cache-versie `v1` → `v2`, zodat bestaande installs de nieuwe shell ophalen), en werken daardoor offline én achter netwerken die CDN's blokkeren. De Chart.js-stack op het admin-dashboard blijft op de CDN (admin-only, niet relevant voor de offline scout-flow). 5 nieuwe tests borgen dat de libs same-origin geserveerd worden, dat `base.html` niet meer naar een CDN wijst, en dat de SW ze pre-cacht.
